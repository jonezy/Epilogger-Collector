...
namespace TweetSharp.Serialization
{
    internal partial class SerializerBase
    {
        ...
        private static void DeserializeMedia(TwitterEntities results, JToken media)
        {
            var value = media.ToString();
            if (value.Equals("Null", StringComparison.InvariantCultureIgnoreCase))
            {
                return;
            }
            var inner = media["media"];
            if (inner.Type == JTokenType.Array)
            {
                var array = JArray.Parse(inner.ToString());
                ...
                case JTokenType.Object:
                    {
                        ...
                        results.Media.Add(ParseMedia(indices, item));
                    }
                    break;
            }
        }

        private static TwitterMedia ParseMedia(IEnumerable<int> indices, JToken item)
        {
            return new TwitterMedia
            {
                Indices = new List<int>(indices),
                Id = Convert.ToInt64(item["id"] != null ? item["id"].ToString().Replace("\"", "") : "0"),
                IdStr = item["id_str"] != null ? item["id_str"].ToString().Replace("\"", "") : null,
                MediaURL = item["media_url"] != null ? item["media_url"].ToString().Replace("\"", "") : null,
                MediaURLHTTPS = item["media_url_https"] != null ? item["media_url_https"].ToString().Replace("\"", "") : null,
                ExtractedURL = item["url"] != null ? item["url"].ToString().Replace("\"", "") : null,
                DisplayURL = item["display_url"] != null ? item["display_url"].ToString().Replace("\"", "") : null,
                ExpandedURL = item["expanded_url"] != null ? item["expanded_url"].ToString().Replace("\"", "") : null,
                MediaType = item["type"] != null ? item["type"].ToString().Replace("\"", "") : null
            };
        }
